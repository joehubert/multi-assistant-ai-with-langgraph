...
In the next chapter, we'll dive deeper into the process of creating AI analyst personas, exploring how we use LLMs to generate diverse perspectives on research topics. in outer state for Send() API
```
...
class InterviewState(MessagesState):
    ...
    analyst: Analyst # Analyst asking questions
    interview: str # Interview transcript
    sections: list # Final key we duplicate
